...
import { allergenOptions, type AllergenId } from "@/lib/allergens";
...
export function PreferencesForm({ locale }: { locale: SiteLocale }) {
  ...
  const [saved, setSaved] = useState<"idle" | "persistent" | "memory">("idle");
  const [hasInteracted, setHasInteracted] = useState(false);
  useEffect(() => {
    const timer = window.setTimeout(() => {
      if (hasInteracted) return;
      const stored = readBrowserJson("local", storageKey, defaultPreferences);
      setPreferences(finderCriteriaFromStored(stored, []));
    }, 0);
  ...
  const toggleAllergen = (id: AllergenId) => update("excludedAllergens", excluded.includes(id) ? excluded.filter((item) => item !== id) : [...excluded, id]);
  function save() { setSaved(writeBrowserJson("local", storageKey, preferences) ? "persistent" : "memory"); }
  ...
  return <section className="preferences-form"><fieldset><legend>{en ? "Default priority" : "Standard-Priorität"}</legend><div className="segmented-options" role="group" aria-label={en ? "Default priority" : "Standard-Priorität"}>{goals.map((goal,index) => <button aria-pressed={preferences.goal === goal.value} key={goal.value} onClick={() => update("goal", goal.value)} type="button">{goalLabels[index]}</button>)}</div></fieldset><fieldset><legend>{en ? "Persistent exclusions" : "Dauerhafte Ausschlüsse"}</legend><div className="preferences-toggle-grid"><label><input checked={Boolean(preferences.veganOnly)} onChange={(event) => update("veganOnly", event.target.checked)} type="checkbox" />{en ? "Vegan only" : "Nur vegan"}</label><label><input checked={Boolean(preferences.additiveFree)} onChange={(event) => update("additiveFree", event.target.checked)} type="checkbox" />{en ? "No common additives" : "Ohne typische Zusatzstoffe"}</label><label><input checked={Boolean(preferences.sweetenerFree)} onChange={(event) => update("sweetenerFree", event.target.checked)} type="checkbox" />{en ? "No sweeteners" : "Ohne Süßungsmittel"}</label><label><input checked={Boolean(preferences.palmOilFree)} onChange={(event) => update("palmOilFree", event.target.checked)} type="checkbox" />{en ? "No palm oil" : "Ohne Palmöl"}</label></div></fieldset><fieldset><legend>{en ? "Exclude allergens by default" : "Allergene standardmäßig ausschließen"}</legend><div className="check-chip-grid">{allergenOptions(locale).map((allergen) => <label key={allergen.id}><input checked={excluded.includes(allergen.id)} onChange={() => toggleAllergen(allergen.id)} type="checkbox" /><span>{allergen.label}</span></label>)}</div><p className="filter-disclaimer">{en ? "This is a convenience filter, not an allergy guarantee. Always check the package." : "Die Einstellung ist eine Hilfe, keine Allergiegarantie. Verpackung immer prüfen."}</p></fieldset><div className="preferences-actions"><button className="primary-button" onClick={save} type="button">{en ? "Save preferences" : "Einstellungen speichern"}</button>{saved !== "idle" ? <span role="status">{saved === "persistent" ? (en ? "Saved in this browser" : "In diesem Browser gespeichert") : (en ? "Saved for this session because browser storage is unavailable" : "Für diese Sitzung gespeichert, da der Browserspeicher nicht verfügbar ist")}</span> : null}</div></section>;
}
