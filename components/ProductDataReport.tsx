      ...
      <div className="product-data-report-body">
        ...
        {state === "success" ? <div className="report-success" role="status"><strong>{c("Danke für den Hinweis.", "Thanks for the report.")}</strong><span>{c("Wir haben ihn zur Datenprüfung vorgemerkt.", "It has been added to the data review queue.")}</span></div> : (
          <form onSubmit={submit}>
            ...
            <div><button disabled={state === "sending"} type="submit">{state === "sending" ? c("Wird gesendet …", "Sending …") : c("Hinweis senden", "Send report")}</button><small>{c("Keine Mailadresse, kein Konto und keine Daten von Barcode oder Kamera.", "No email address, account, barcode, or camera data.")}</small></div>
