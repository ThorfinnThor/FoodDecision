...
import { alternativeGoalOrder, rankImprovingAlternatives, type AlternativeGoal, type AlternativeRecommendation } from "@/lib/product-insights";
...
export default async function ProductPage({ params }: Props) {
  ...
  return <main className="product-page">
    ...
    <section className="detail-section ingredients-section"><div className="section-heading"><p className="eyebrow">{c("Zutaten & Allergene", "Ingredients and allergens")}</p><h2>{c("Was du vor dem Kauf wissen solltest", "What to know before buying")}</h2></div><IngredientCheck product={product} /><div className="two-column"><div className="detail-panel"><h3>{c("Zutaten", "Ingredients")}</h3>{product.ingredients.length ? <ul className="tag-list">{product.ingredients.map((item) => <li key={item}>{item}</li>)}</ul> : <p>{c("Keine verlässliche Zutatenliste verfügbar.", "No reliable ingredient list available.")}</p>}</div><div className="detail-panel"><h3>{c("Bekannte Allergene", "Known allergens")}</h3><ul className="tag-list warning-tags">{(product.allergens.length ? product.allergens : [c("Keine bestätigten Allergendaten", "No confirmed allergen data")]).map((item) => <li key={item}>{item}</li>)}</ul><p className="small-note">{c("Bei Allergien zählt immer die aktuelle Verpackungsangabe.", "For allergies, always rely on the current package label.")}</p></div></div></section>
