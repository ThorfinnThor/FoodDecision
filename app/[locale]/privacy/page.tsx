...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: pick(locale, "Datenschutz - Food Decision Engine", "Privacy - Food Decision Engine"),
    description: pick(locale, "Wie Food Decision Engine Kamera-, lokale und freiwillig übermittelte Daten verarbeitet.", "How Food Decision Engine handles camera, local, and voluntarily submitted data."),
    ...
    <section className="section privacy-section">
      ...
      <div className="privacy-content">
        <article id="camera">
          ...
          <h2>{c("Kamera und Barcode-Scanner", "Camera and barcode scanner")}</h2>
          <p>{c("Die Kamera wird ausschließlich nach einem Klick auf „Barcode mit Kamera scannen“ angefordert. Der Browser verarbeitet das Kamerabild lokal, um einen Barcode zu erkennen. Weder Kamerabilder noch Video, Ton oder die erkannte Barcodenummer werden an Food Decision Engine, Supabase, Vercel oder andere Dritte übertragen.", "Camera access is requested only after you select “Scan barcode with camera.” Your browser processes the camera image locally to recognize a barcode. Camera images, video, audio, and the recognized barcode number are not sent to Food Decision Engine, Supabase, Vercel, or any other third party.")}</p>
          <p>{c("Die Kamera stoppt nach einem Treffer, beim manuellen Stoppen, beim Wechsel in einen anderen Tab und beim Verlassen der Seite. Wir fordern keinen Mikrofonzugriff an. Deine Kameraberechtigung verwaltest und widerrufst du jederzeit in den Website-Einstellungen deines Browsers.", "The camera stops after a match, when stopped manually, when you switch tabs, and when you leave the page. We never request microphone access. You can manage or revoke camera permission at any time in your browser's site settings.")}</p>
          <p>{c("Der optionale Scanverlauf enthält Barcodenummer und Produktname und wird ausschließlich lokal in deinem Browser gespeichert. Du kannst ihn auf der Scanner-Seite einzeln oder über die Einstellungen oben vollständig löschen.", "Optional scan history contains the barcode number and product name and is stored only in your browser. You can clear it on the scanner page or delete it with the controls above.")}</p>
        </article>

        <article>
          <p className="eyebrow">02</p>
          <h2>{c("Lokale Funktionen", "Local features")}</h2>
          <p>{c("Favoriten, Einkaufsliste, abgehakte Einkäufe, Finder-Präferenzen und Scanverlauf verwenden den lokalen Browserspeicher. Diese Angaben verlassen dein Gerät nicht und sind nicht mit einem Konto verknüpft. Food Decision Engine setzt für diese Funktionen keine Cookies.", "Favorites, shopping lists, completed shopping items, Finder preferences, and scan history use local browser storage. This data does not leave your device and is not linked to an account. Food Decision Engine does not set cookies for these features.")}</p>
        ...
        <article>
          ...
          <p>{c("Die optionale Nutzungsstatistik ist standardmäßig deaktiviert. Erst nach deiner Aktivierung senden wir ausgewählte Ereignisse wie das Öffnen einer Produktseite, den Abschluss des Finders oder einen Vergleich. Suchbegriffe, URL-Suchparameter, Kameradaten und Barcodes werden nicht erfasst.", "Optional usage analytics is disabled by default. Only after you enable it do we send selected events such as opening a product page, completing the Finder, or comparing products. Search terms, URL query parameters, camera data, and barcodes are not collected.")}</p>
        ...
        <article>
          ...
          <h2>{c("Newsletter", "Newsletter")}</h2>
          <p>{c("Wenn du dich freiwillig für Produktupdates anmeldest, übertragen und speichern wir deine E-Mail-Adresse, Sprache, Quelle der Anmeldung und Zeitpunkt deiner Einwilligung in unserer Supabase-Datenbank. Die Newsletter-Anmeldung ist von Scanner und Nutzungsstatistik unabhängig. Eine Anmeldung erfolgt nur mit aktiv angekreuzter Einwilligung.", "If you voluntarily subscribe to product updates, we send and store your email address, language, signup source, and consent time in our Supabase database. Newsletter signup is separate from the scanner and usage analytics. Signup requires an actively selected consent checkbox.")}</p>
        </article>

        <article>
          <p className="eyebrow">05</p>
          <h2>{c("Hosting und Dienstleister", "Hosting and service providers")}</h2>
          <p>{c("Die Website wird über Vercel ausgeliefert. Dabei können technisch notwendige Verbindungsdaten wie IP-Adresse, Zeitpunkt, angeforderte URL und Browserinformationen in Infrastruktur- und Sicherheitsprotokollen verarbeitet werden. Strukturierte Newsletter- und freiwillige Analysedaten werden serverseitig in Supabase gespeichert. Geheime Datenbankschlüssel werden nie an den Browser ausgeliefert.", "The website is delivered through Vercel. Technically necessary connection data such as IP address, time, requested URL, and browser information may be processed in infrastructure and security logs. Structured newsletter and voluntary analytics data is stored server-side in Supabase. Secret database keys are never delivered to the browser.")}</p>
        ...
        <article>
          ...
          <p>{c("Produktdaten und Produktbilder stammen von Open Food Facts. Die Daten werden vor dem Seitenaufruf importiert; Produktseiten rufen keine Open-Food-Facts-API live auf. Produktbilder können direkt von den erlaubten Open-Food-Facts-Bildservern geladen werden, wodurch dort technisch deine IP-Adresse verarbeitet werden kann. Externe Quellen- und Händlerlinks öffnen erst nach deinem Klick.", "Product data and product images come from Open Food Facts. Data is imported before page load; product pages do not call the Open Food Facts API live. Product images may load directly from approved Open Food Facts image servers, which can technically process your IP address. External source and merchant links open only after you select them.")}</p>
        </article>
        ...
        <article>
          ...
          {operatorName && contact
            ...
            : <p className="privacy-contact-warning">{c("Vor dem öffentlichen Launch müssen hier noch der vollständige Betreibername und eine erreichbare Datenschutz-Kontaktadresse ergänzt werden.", "Before public launch, the operator's full legal name and a working privacy contact address must be added here.")}</p>}
        </article>
      </div>
    </section>
  </main>;
