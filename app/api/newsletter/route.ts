...
export async function POST(request: Request) {
  ...
  let input: { email?: unknown; website?: unknown; source?: unknown; consent?: unknown; locale?: unknown };
  ...
  const locale = input.locale === "en-US" ? "en-US" : "de-DE";
  const response = await supabaseServerRequest("newsletter_subscribers?on_conflict=email_normalized", {
    method: "POST",
    headers: { Prefer: "resolution=merge-duplicates,return=minimal" },
    body: JSON.stringify({ email, email_normalized: normalized, locale, source: typeof input.source === "string" ? input.source.slice(0, 80) : "homepage", status: "pending", consented_at: new Date().toISOString() }),
  });
